    ...
    <div style="font-family: system-ui, sans-serif; background:#f0fdf4; padding:32px;">
      <div style="max-width:520px; margin:0 auto; background:#ffffff; border-radius:12px; padding:32px;">
        ...
        <p style="color:#374151; margin:0 0 20px;">
          <strong>${data.commenterName}</strong> comentó en
        ...
        <a href="${data.taskUrl}" style="display:inline-block; margin-top:24px; background:#16a34a; color:#ffffff; text-decoration:none; padding:12px 20px; border-radius:8px; font-weight:600;">
          Ver la tarea
        </a>
      </div>
    </div>
...
export async function sendRegistrationEmail(to: string, data: RegistrationEmail) {
  ...
  const html = `
    <div style="font-family: system-ui, sans-serif; background:#f0fdf4; padding:32px;">
      <div style="max-width:520px; margin:0 auto; background:#ffffff; border-radius:12px; padding:32px;">
        <h2 style="color:#166534; margin:0 0 8px;">Bienvenido a Jilite, ${data.name}</h2>
        ...
        <a href="${data.verifyUrl}" style="display:inline-block; background:#16a34a; color:#ffffff; text-decoration:none; padding:12px 20px; border-radius:8px; font-weight:600;">
          Verificar mi correo
        </a>
        <p style="color:#6b7280; margin:24px 0 0; font-size:13px;">
          Si no puedes ver el botón, copia este enlace en tu navegador:
          <br /><a href="${data.verifyUrl}" style="color:#166534;">${data.verifyUrl}</a>
        </p>
      </div>
    </div>
  `;
...
export async function sendPasswordResetEmail(to: string, data: PasswordResetEmail) {
  ...
  const html = `
    <div style="font-family: system-ui, sans-serif; background:#f0fdf4; padding:32px;">
      <div style="max-width:520px; margin:0 auto; background:#ffffff; border-radius:12px; padding:32px;">
        <h2 style="color:#166534; margin:0 0 8px;">Hola, ${data.name}</h2>
        <p style="color:#374151; margin:0 0 16px;">
        ...
        <a href="${data.resetUrl}" style="display:inline-block; background:#16a34a; color:#ffffff; text-decoration:none; padding:12px 20px; border-radius:8px; font-weight:600;">
          Restablecer contraseña
        </a>
        <p style="color:#6b7280; margin:24px 0 0; font-size:13px;">
          Si no puedes ver el botón, copia este enlace en tu navegador:
          <br /><a href="${data.resetUrl}" style="color:#166534;">${data.resetUrl}</a>
        </p>
      </div>
    </div>
  `;
  ...
  const html = `
    <div style="font-family: system-ui, sans-serif; background:#f0fdf4; padding:32px;">
      <div style="max-width:520px; margin:0 auto; background:#ffffff; border-radius:12px; padding:32px;">
        <h2 style="color:#166534; margin:0 0 8px;">Hola, ${data.name}</h2>
        <p style="color:#374151; margin:0 0 16px;">
          Te asignaron una nueva tarea en <strong>${data.projectName}</strong>:
        </p>
        <div style="background:#f0fdf4; border-left:4px solid #22c55e; padding:16px; border-radius:8px; color:#374151;">
          <strong>${data.taskTitle}</strong>
        </div>
